...
# ── 2. Read data ──────────────────────────────────────────────────────────────
# Sheet layout:
#   Row 1 : group labels  — cols 1-3 empty, cols 4+ = "PD" / "Control"
#   Row 2 : column names  — Accession, Gene symbol, p-value, CSF01..CSF80
#   Row 3+: protein data  (log2-normalized values, already processed)

# Step A: group labels from row 1 (read separately to avoid any header issues)
row1 <- read_excel(INPUT_FILE, sheet = "Normalized",
                   col_names = FALSE, n_max = 1)
# n_cols_total includes the 3 metadata cols + 80 sample cols
n_cols_total <- ncol(row1)
group_labels <- unlist(row1[1, 4:n_cols_total], use.names = FALSE)
...
cat(sprintf("Groups — Healthy: %d  PD: %d\n",
            sum(groups == "Healthy"), sum(groups == "PD")))

# Step B: protein data with row 2 as header (default readxl behaviour)
raw <- read_excel(INPUT_FILE, sheet = "Normalized", skip = 1)
cat(sprintf("Proteins loaded: %d\n", nrow(raw)))

# Metadata vectors (by column position — avoids column-name encoding issues)
accessions   <- as.character(raw[[1]])   # col 1 = Accession
gene_symbols <- as.character(raw[[2]])   # col 2 = Gene symbol

# Sample columns: positions 4 to n_cols_total
sample_cols <- names(raw)[4:n_cols_total]
...
# Use integer rownames (1..n) — avoids ALL rowname lookup / deduplication bugs.
# Gene and Accession info is kept in parallel vectors that mirror mat rows.
mat <- as.matrix(raw[, sample_cols])
storage.mode(mat) <- "numeric"           # coerce to numeric in place
rownames(mat) <- seq_len(nrow(mat))      # plain integers, no name collisions

cat(sprintf("PON1 row index: %s\n",
            paste(which(gene_symbols == "PON1"), collapse = ",")))

# Remove rows with any NA — apply the same logical mask to all parallel vectors
...
cat(sprintf("PON1 in filtered set: %s\n\n",
...
# topTable with sort.by="none" preserves the exact row order of mat.
# We attach gene info by positional alignment — no rowname lookup needed.
tt <- topTable(fit2, coef = "PD_vs_Healthy",
               number = Inf, sort.by = "none")
stopifnot(nrow(tt) == length(gene_keep))   # must match
...
cat(sprintf("PON1 in results: %s\n\n",
            ifelse(any(results$is_PON1), "YES", "NO")))

# PON1 result summary
pon1_row <- filter(results, is_PON1)
if (nrow(pon1_row) == 0) stop("PON1 not found — check input file path.")
cat(sprintf("PON1 | log2FC=%+.3f | p=%.4f | adj.p=%.4f | %s\n\n",
...
fmt_pval <- function(p) {
  if (p < 0.001) "p < 0.001" else
    if (p < 0.01)  "p < 0.01"  else sprintf("p = %.3f", p)
}
fmt_adjp <- function(p) {
  if (p < 0.001) "adj.p < 0.001" else
    if (p < 0.01)  "adj.p < 0.01"  else sprintf("adj.p = %.3f", p)
}

# ── Plot 1: PON1 Boxplot ──────────────────────────────────────────────────────
cat("[Plot 1] PON1 boxplot\n")

pon1_idx  <- which(gene_keep == "PON1")[1]
pon1_expr <- as.numeric(mat[pon1_idx, ])
pon1_dat  <- tibble(Expression = pon1_expr, Group = groups)

rng   <- diff(range(pon1_expr, na.rm = TRUE))
y_max <- max(pon1_expr, na.rm = TRUE)
y_seg <- y_max + rng * 0.08
y_lbl <- y_seg + rng * 0.10
y_lim <- y_lbl + rng * 0.22

sig_label <- paste0(fmt_pval(pon1_row$pval), "\n", fmt_adjp(pon1_row$adj_pval))
...
p1 <- ggplot(pon1_dat, aes(x = Group, y = Expression, fill = Group)) +
  ...
  annotate("segment",
           x = 1, xend = 2, y = y_seg, yend = y_seg,
           linewidth = 0.8, color = "black") +
  annotate("segment",
           x = 1, xend = 1,
           y = y_max + rng * 0.01, yend = y_seg,
           linewidth = 0.8, color = "black") +
  annotate("segment",
           x = 2, xend = 2,
           y = y_max + rng * 0.01, yend = y_seg,
           linewidth = 0.8, color = "black") +
  annotate("text", x = 1.5, y = y_lbl,
           label = sig_label, size = 4.0, fontface = "bold", lineheight = 1.4) +
  ...
  labs(title    = "PON1 Expression in CSF — Healthy vs PD",
       subtitle = "limma | log2 Normalized Intensity",
       x = NULL,
       y = expression(log[2] ~ "Normalized Intensity")) +
  BASE_THEME

ggsave(file.path(OUTPUT_DIR, "plot1_PON1_boxplot.pdf"), p1, width = 5, height = 6.5)
ggsave(file.path(OUTPUT_DIR, "plot1_PON1_boxplot.png"), p1, width = 5, height = 6.5, dpi = 180)
...
vol_dat <- results %>%
  mutate(
    ...
    dot_size  = if_else(is_PON1, 5.0, 1.8),
    dot_alpha = if_else(is_PON1, 1.0, 0.55)
  ) %>%
  arrange(is_PON1)   # draw PON1 on top

color_scale <- c(PON1 = COL_PON1, UP = COL_UP, DOWN = COL_DOWN, NS = COL_NS)

pon1_vol   <- filter(vol_dat, is_PON1)
pon1_label <- sprintf("PON1\nlog2FC = %+.3f\n%s\n%s",
                      pon1_vol$log2FC,
...
p2 <- ggplot(vol_dat, aes(x = log2FC, y = log10p)) +
  ...
  geom_vline(xintercept = c(-0.58, 0.58), linetype = "dashed",
             color = "grey45", linewidth = 0.55) +
  geom_point(aes(color = dot_color, size = dot_size, alpha = dot_alpha)) +
  geom_label_repel(
    data          = pon1_vol,
    aes(label     = pon1_label),
    size          = 3.8, fontface = "bold",
    fill          = "#FFF9E6", color = COL_PON1,
    box.padding   = 1.2, point.padding = 0.7,
  ...
  labs(
    ...
    subtitle = sprintf("limma | %d proteins | PON1 highlighted", nrow(results)),
...
ggsave(file.path(OUTPUT_DIR, "plot2_volcano_PON1.pdf"), p2, width = 8, height = 6.5)
ggsave(file.path(OUTPUT_DIR, "plot2_volcano_PON1.png"), p2, width = 8, height = 6.5, dpi = 180)
